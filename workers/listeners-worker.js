...
// - НЕ использует Icecast listeners
//
// РАЗМЕЩЕНИЕ:
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST') {
      try {
        ...
        const { user_id, first_name, last_name, username, action, isAdmin } = data;
        ...
        // =====================================================
        if (action === 'open') {
          console.log(`[LISTENER] Register: user_id=${user_id}, name=${first_name}, admin=${isAdmin}`);
          
          // SQL: Вставляем или обновляем слушателя
          // ON CONFLICT позволяет обновить существующую запись
          await env.DB.prepare(`
            INSERT INTO listeners (user_id, first_name, last_name, username, is_admin, last_heartbeat)
            VALUES (?, ?, ?, ?, ?, ?)
        ...
        // =====================================================
        // ПОЛУЧЕНИЕ ОБЩЕГО КОЛИЧЕСТВА СЛУШАТЕЛЕЙ
        // =====================================================
    ...
    if (request.method === 'GET') {
      try {
        ...
        const listeners = await env.DB.prepare(`
          SELECT user_id, first_name as name FROM listeners
        `).all();
        ...
        console.log(`[LISTENER] GET: total=${total}`);
